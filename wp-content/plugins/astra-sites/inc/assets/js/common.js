( function ( $ ) {
	...
	AstraImageCommon = {
		...
		_bind: function () {
			...
			$( document ).on(
				'ast-image__refresh',
				AstraImageCommon._initImages
			);
			$( document ).on(
				'ast-image__set-scope',
				AstraImageCommon._setScope
			);
			$( document ).on(
				'click',
				'.ast-image__list-img-overlay',
				AstraImageCommon._preview
			);
			$( document ).on(
				'click',
				'.ast-image__go-back-text',
				AstraImageCommon._goBack
			);
			$( document ).on(
				'click',
				'.ast-image__save',
				AstraImageCommon._save
			);
			$( document ).on(
				'click',
				'.ast-image__filter-safesearch input',
				AstraImageCommon._filter
			);
			$( document ).on(
				'change',
				'.ast-image__filter select',
				AstraImageCommon._filter
			);
			$( document ).on(
				'click',
				'.ast-image__edit-api',
				AstraImageCommon._editAPI
			);
			$( document ).on(
				'click',
				'.ast-image__browse-images',
				AstraImageCommon._browse
			);
			$( document ).on(
				'click',
				'.ast-image__download-icon',
				AstraImageCommon._saveFromScreen
			);
		},
		...
		_saveFromScreen: function () {
			...
			AstraImageCommon._saveAjax( function ( data ) {
				...
				wp.media.view.AstraAttachmentsBrowser.object.photoUploadComplete(
					data.data
				);
				saveIcon.text( 'Done' );
		...
		_filter: function () {
			let safesearch = $scope.find(
				'.ast-image__filter-safesearch input:checked'
			).length
				? true
				: false;
			let category = $scope
				.find( '.ast-image__filter-category select' )
				.val();
			let orientation = $scope
				.find( '.ast-image__filter-orientation select' )
				.val();
		...
		_goBack: function () {
			...
			$scope
				.find( '.ast-image__skeleton-inner-wrap' )
		...
		_preview: function ( event ) {
			if (
				event &&
				event.target.classList.contains( 'ast-image__download-icon' )
			) {
				return;
			}
			...
			$scope
				.find( '.ast-image__search-wrap' )
				.before( $( '#tmpl-ast-image-go-back' ).text() );
		...
		_setScope: function () {
			AstraImageCommon.frame =
				wp.media.view.AstraAttachmentsBrowser.object.$el.closest(
					'.media-frame'
				);
			...
			if ( ! $scope.find( '.ast-image__filter-wrap' ).length ) {
				$scope
					.find( '.ast-attachments-search-wrap' )
					.append( pixabay_filter() );
			}
			...
			$scope
				.find( '.ast-image__skeleton-inner-wrap' )
				.scroll( AstraImageCommon._loadMore );
		...
		_initImages: function () {
			...
			if ( AstraImageCommon.infiniteLoad ) {
				AstraImageCommon.images.push(
					wp.media.view.AstraAttachmentsBrowser.images
				);
				$scope.find( '.ast-image__skeleton' ).append( list_html );
			} else {
				AstraImageCommon.images =
					wp.media.view.AstraAttachmentsBrowser.images;
		...
		_loadMore: function () {
			...
			if (
				undefined !=
				$scope.find( '.ast-image__list-wrap:last' ).offset()
			) {
				if (
					$( window ).scrollTop() + AstraImageCommon.offset >=
					$scope.find( '.ast-image__list-wrap:last' ).offset().top
				) {
