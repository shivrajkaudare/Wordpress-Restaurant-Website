			...
			if ( undefined != wp && wp.media ) {
				...
				const pixabayFrame = {
					...
					browseRouter( routerView ) {
						oldMediaFrameSelect.prototype.browseRouter.apply(
							this,
							arguments
						);
						routerView.set( {
					...
					bindHandlers() {
						if ( astraImages.is_elementor_editor ) {
							oldMediaFramePost.prototype.bindHandlers.apply(
								this,
								arguments
							);
						} else {
							oldMediaFrameSelect.prototype.bindHandlers.apply(
								this,
								arguments
							);
						}
						this.on(
							'content:create:astraimages',
							this.astraimages,
							this
						);
					},
					...
					astraimages( contentRegion ) {
						...
						let thisView =
							new wp.media.view.AstraAttachmentsBrowser( {
								controller: this,
								model: state,
				...
				if ( astraImages.is_elementor_editor ) {
					wp.media.view.MediaFrame.Post =
						oldMediaFramePost.extend( pixabayFrame );
				} else {
					wp.media.view.MediaFrame.Select =
						oldMediaFrameSelect.extend( pixabayFrame );
				}
			}
		},
	};
		...
		if ( astraImages.is_bb_active && astraImages.is_bb_editor ) {
			if ( undefined !== FLBuilder ) {
				if ( null !== FLBuilder._singlePhotoSelector ) {
					FLBuilder._singlePhotoSelector.on(
						'open',
						function ( event ) {
							AstraImages.init();
						}
